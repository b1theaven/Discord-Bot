...
module.exports = {
  ...
  description: "Block spam mentions",
  permission: "ADMINISTRATOR",
  options: [
    {
      name: 'number',
      description: 'The number of mentions you wanna block',
      type: "NUMBER",
      required: true
    },
    {
  ...
  execute: async(client, interaction, args) => {
    ...
    //if(!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) return await interaction.reply({ content: 'You do not have a permission to setup automod within thi server', ephemeral: true})
        ...
        setTimeout(async () => {
          ...
          ---------------------------------------------
          Total mentions ${num} will be blocked. Thank you`)
